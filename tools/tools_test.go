...
func TestInvoiceService_ToolCreation(t *testing.T) {
	...
	t.Run("create_invoice_tool", func(t *testing.T) {
		tool := service.CreateInvoiceTool()
		assert.Equal(t, "lnc_create_invoice", tool.Name)
		...
		assert.Equal(t, 1, amountField["minimum"])
	})
...
func TestServiceIntegration(t *testing.T) {
	t.Run("invoice_service_complete", func(t *testing.T) {
		...
		assert.NotEmpty(t, createTool.Name)
